...
import { bulkUpdate, createTaskSchema, getTaskSchema } from "../schemas";
...
const app = new Hono()
  .get(
    ...
    async (c) => {
      ...
      const { workspaceId, status, assigneeId, dueDate, projectId, search } =
        c.req.valid("query");
  ...
  .patch(
    ...
    async (c) => {
      const user = c.get("user");
      const databases = c.get("databases");
      const { name, status, assigneeId, dueDate, description, projectId } =
        c.req.valid("json");
